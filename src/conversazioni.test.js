...

{
  ...
  // Esc si chiama sempre «indietro» e canc sempre «esci», in ogni schermata:
  // nominarli col posto dove portano costringeva a rileggerli ogni volta.
  assert.match(righe[righe.length - 1], /esc indietro/, 'esc dice sempre indietro');
  assert.match(righe[righe.length - 1], /canc esci/, 'e canc sempre esci');
  assert.ok(
    righe.every((r) => r.length <= 90),
